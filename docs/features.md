...
| Browser support floor and capability gating | `confirmed`, **Safari half unverified** | Shipped in M5 (D-016). Chromium and Firefox run the whole suite; **Playwright's Linux WebKit ships no OPFS at all**, so the Safari half of the floor rests on documented feature availability plus the gate rather than on a run (RE-022). What that engine does verify is the gate itself, firing for real on a browser that genuinely fails. The probe *calls* a synchronous method and reads what came back, because Safari 15.2–16.3 has `createSyncAccessHandle` and returns Promises from the handle's methods — so every interface check passes and the import dies inside WASM. The gate runs before the manifest, names the specific missing capability *and* the floor, and says nothing was fetched or changed. |
