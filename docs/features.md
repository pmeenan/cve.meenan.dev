...
| Model orchestrates, never transcribes | `confirmed` | D-044. Aggregates may enter model context; row sets from the curated tools travel as handles rendered straight from SQLite. Narrowed once, deliberately: the `SELECT`-only tool gets a bounded window of its own result, because a model that writes `SELECT max(...)` and is told only "1 row returned" cannot answer its own question (D-078). Numbers come from queries, never token sampling. |
