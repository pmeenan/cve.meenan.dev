...
| Model reads every tool result, bounded | `confirmed` | D-087 (reversing D-044's "never transcribes"). Aggregates, record-search rows and SQL rows enter model context in a bounded window — 50 rows within 12,000 characters, what lies outside it counted (D-078's window, generalised) — and a whole record for `cve_detail`. Every result is also rendered whole by the fixed UI, straight from SQLite; the model may not state a number that did not come from a tool. |
| Compute over the last result, sandboxed | `confirmed` | D-088. A `compute` tool runs a model-written JavaScript function over the *full* rows of the most recent result — aggregate, record search or SQL, whoever ran it — inside an opaque-origin `srcdoc` frame with a `default-src 'none'` CSP and a blob: worker terminated at 10 s; output clipped to the model budget; the code and output rendered as a chat step. `window.cveExplorer.last()` hands an agent the same rows whole. The boundary is proved from inside the sandbox on both engines (`tests/e2e/compute.spec.ts`). |
...
| `SELECT`-only SQL tool for capable models | `confirmed` | D-044. Row-capped, timed out, **and byte-capped** — the row cap alone bounds nothing, since one row can be the whole text table (D-078). The console shares all three. |
| Site-hosted model tier — Ollama behind a restricted same-origin endpoint | `confirmed` | D-057. **First tier to ship (M7).** Server-pinned model (`qwen3:8b` since 2026-08-09, selected by the D-046 benchmark), chat completion only, rate- and concurrency-limited, nothing stored, no body logging. The question and tool results transit our server — disclosed at first use. |
...
| Client-side telemetry | `rejected (D-009)` | No collection of any kind *by the app*. The origin keeps an ordinary access log with real visitor addresses; the claim is the structural one — the corpus and every query over it run in the browser (D-079). |
